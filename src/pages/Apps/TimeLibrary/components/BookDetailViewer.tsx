...
import { Drawer, Spin, Tabs, Typography, Divider, Button, Space, Form, Input, InputNumber, Message, Upload } from '@arco-design/web-react';
          ...
          <Tabs defaultActiveTab="1">
              ...
              {!editMode ? (
                <div style={{ marginTop: 20 }}>
                  <Space style={{ marginBottom: 20 }}>
                    <Button type="primary" icon={<IconEdit />} onClick={() => setEditMode(true)}>编辑书籍信息</Button>
                  </Space>
                  <Typography>
                    ...
                    <FormItem label="封面图片" field="cover_url">
                      <Upload
                        ...
                        fileList={form.getFieldValue('cover_url') ? [{
                          uid: '-1',
                          url: detail.cover_url || undefined, // 这里用详情里的完整 URL
                          status: 'done'
                        }] : []}
            ...
            <TabPane key="2" title="AI 人设">
              {!personaEditMode ? (
                <div style={{ marginTop: 20 }}>
                  <Space style={{ marginBottom: 20 }}>
                    <Button type="primary" icon={<IconEdit />} onClick={handleEditPersona}>配置/编辑人设</Button>
                  </Space>
                  {detail.ai_persona ? (
                    <Typography>
                      ...
                      <Paragraph style={{ whiteSpace: 'pre-wrap' }}>
                        {detail.ai_persona.system_prompt}
                      </Paragraph>
                    </Typography>
                  ) : (
                ...
                <div style={{ marginTop: 20 }}>
                  <Form form={personaForm} layout="vertical">
                    ...
                    <FormItem label="人设系统提示词 / 描述" field="system_prompt" rules={[{ required: true, message: '请输入人设系统提示词' }]}>
                      <Input.TextArea placeholder="请输入告诉 AI 它应该扮演什么角色的详细提示词" autoSize={{ minRows: 4, maxRows: 10 }} />
                    </FormItem>
                    <FormItem label="角色头像" field="avatar_url">
                      <Upload
                        ...
                        fileList={personaForm.getFieldValue('avatar_url') ? [{
                          uid: '-1',
                          url: detail.ai_persona?.avatar_url || undefined,
                          status: 'done'
                        }] : []}
            ...
            <TabPane key="3" title={`章节内容 (${detail.contents?.length || 0})`}>
              {!chapterAddMode ? (
                <div style={{ marginTop: 20 }}>
                  <Space style={{ marginBottom: 20 }}>
                    <Button type="primary" icon={<IconPlus />} onClick={handleAddChapter}>追加新章节</Button>
                  </Space>
                  {detail.contents && detail.contents.length > 0 ? (
                    <div>
                      {detail.contents.sort((a, b) => a.order - b.order).map((chapter, index) => (
                        <div key={chapter.id} style={{ marginBottom: 30 }}>
                          <Title heading={4}>
                            第 {index + 1} 章: {chapter.chapter_title}
                          </Title>
                          <Text type="secondary" style={{ fontSize: 12 }}>
                            {dayjs(chapter.created_at).format('YYYY-MM-DD HH:mm')} · 排序 {chapter.order}
                          </Text>
                ...
                <div style={{ marginTop: 20 }}>
                  <Form form={chapterForm} layout="vertical">
                    <FormItem label="章节标题" field="chapter_title" rules={[{ required: true, message: '请输入章节标题' }]}>
                      <Input placeholder="请输入章节标题" />
                    </FormItem>
                    <FormItem label="章节正文" field="content" rules={[{ required: true, message: '请输入章节正文' }]}>
